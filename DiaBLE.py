# -*- coding: utf-8 -*-
"""
Created on Wed Dec 11 10:54:04 2024
...
# ============================================================================
def GraphVisualizerForDiable(Graph_To_Visualize,
                             set_of_seeds, 
                             set_of_neighbors,
                             set_of_neighbors_of_neighbors):
    plt.show()
    pos = nx.kamada_kawai_layout(Graph_To_Visualize)
    nx.draw_networkx_nodes(Graph_To_Visualize, pos, node_color="green",
                           node_size = 10, alpha=0.7)
    nx.draw_networkx_nodes(Graph_To_Visualize, pos, nodelist = list(set_of_seeds), 
                           node_color = "red", node_size = 25, 
                           edgecolors = "red")
    nx.draw_networkx_nodes(Graph_To_Visualize, pos, nodelist = list(set_of_neighbors), 
                           node_color = "orange", node_size = 20, 
                           edgecolors = "orange")
    nx.draw_networkx_nodes(Graph_To_Visualize, pos, nodelist = list(set_of_neighbors_of_neighbors), 
                           node_color = "yellow", node_size = 15, 
                           edgecolors = "yellow")
    nx.draw_networkx_edges(Graph_To_Visualize, pos, width = 0.5, alpha = 0.5)
    plt.tight_layout()
...
def diable_iteration_of_first_X_nodes(G, S, X, alpha):
    ...
    # seeds_and_neighbors = not_in_cluster.copy()
    ...
    while len(added_nodes) < X:
        ...
        next_node = 'nix'
        # reduced_not_in_cluster = \
        #     reduce_not_in_cluster_nodes(all_degrees,neighbors, G, 
        #                                 not_in_cluster, cluster_nodes, alpha)
        reduced_not_in_cluster = \
            reduce_not_in_cluster_nodes(all_degrees,neighbors, G, 
                                        neighbors_of_the_seeds, cluster_nodes, 
                                        alpha)
        ...
        s0 = len(cluster_nodes)
        # previously_not_in_cluster = neighbors_of_the_seeds.copy()
        
        
        # Updating of not_in_cluster set
        ...
        # new_neighbors = not_in_cluster.difference(previously_not_in_cluster)

        # if new_neighbors:
        # # Update of the DiaBLE Universe
...
#
# ===========================================================================
def DiaBLE(G_original, seed_genes, max_number_of_added_nodes,
           alpha, outfile=None):
    """
